...
//   아이디 중복검사
   public boolean checkId(String id) {
      ...
      try {
         con = DBConnection.getConnection();
         pstm = con.prepareStatement(sql);
         pstm.setString(1, id); // ?의 갯수로 1부터 시작
         rs = pstm.executeQuery();
...
//         중복이 없다면 true
   ...
   // 내가 추천한 사람의 아이디와 이름을 확인해보자 > 전체 행이 아닌 일부 행이기 때문에 DTO 만들기
   // 내 정보 + 내가 추천한 사람의 아이디, 이름을 한번에 들고와보자
   ...
   // 나를 추천한 사람이 몇 명인지 확인하는 메소드
   // 매개변수 > pk, return: int
   public int countRecommendr(int pk) {
	   String sql = "SELECT COUNT(*)\r\n"
	   		+ "FROM TBL_USER\r\n"
	   		+ "WHERE USER_RECOMMENDERNUMBER = ?";
	   
	   // select에 대한 결과를 받아오는 변수
	   int count = 0;
	   ...
	   try {
	        ...
	        count = rs.getInt(1);
		        ...
		        e.printStackTrace();
		        System.out.println("countRecommendr() 도중 에러");
		   } finally {
		        try {
		           DBConnection.close(con, pstm, rs);
		        } catch (SQLException e) {
		           e.printStackTrace();
		           System.out.println("countRecommendr() 닫는 도중 에러");
		        }
		   }
	   return count;
   }
